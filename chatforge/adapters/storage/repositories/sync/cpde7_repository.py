"""
Sync CPDE-7 Repository.

Storage and retrieval of CPDE-7 extracted profiling data.

Usage:
    from chatforge.adapters.storage.repositories.sync import CPDE7Repository

    repo = CPDE7Repository(session)
    run = repo.create_run(user_id="user-1", config={"dimensions": "all"})
"""
...
class CPDE7Repository:
    """
    Repository for CPDE-7 extracted data access.
    ...
    def start_run(
        ...
        *,
        ...
        run = self.session.query(ProfilingDataExtractionRun).get(run_id)
        if not run:
    ...
    def complete_run(self, *, run_id: int, duration_ms: int) -> ProfilingDataExtractionRun:
    ...
    def fail_run(self, *, run_id: int, error: str) -> ProfilingDataExtractionRun:
    ...
    def get_run(self, *, run_id: int) -> Optional[ProfilingDataExtractionRun]:
        """Get an extraction run by ID."""
        return self.session.query(ProfilingDataExtractionRun).get(run_id)

    def get_runs_for_user(self, *, user_id: str, limit: int = 20) -> list[ProfilingDataExtractionRun]:
        """Get recent extraction runs for a user."""
        return (
            self.session.query(ProfilingDataExtractionRun)
            .filter(ProfilingDataExtractionRun.user_id == user_id)
            .order_by(ProfilingDataExtractionRun.created_at.desc())
            .limit(limit)
            .all()
        )

    def get_active_run(self, *, chat_id: int) -> Optional[ProfilingDataExtractionRun]:
        """Get any pending or running extraction run for a chat."""
        return (
            self.session.query(ProfilingDataExtractionRun)
            .filter(ProfilingDataExtractionRun.chat_id == chat_id)
            .filter(ProfilingDataExtractionRun.status.in_(["pending", "running"]))
            .first()
        )

    def get_runs_for_chat(self, *, chat_id: int, limit: int = 10) -> list[ProfilingDataExtractionRun]:
    ...
    def get_latest_completed_run(self, *, chat_id: int) -> Optional[ProfilingDataExtractionRun]:
        """Get the most recent completed extraction run for a chat."""
        return (
            self.session.query(ProfilingDataExtractionRun)
            .filter(ProfilingDataExtractionRun.chat_id == chat_id)
            .filter(ProfilingDataExtractionRun.status == "completed")
            .order_by(ProfilingDataExtractionRun.completed_at.desc())
            .first()
        )

    def update_run_status(self, *, run_id: int, status: str) -> Optional[ProfilingDataExtractionRun]:
    ...
    def get_cpde7_data(self, *, user_id: str, limit: int = 100) -> list[ExtractedProfilingData]:
        """Get all CPDE-7 extracted data for a user."""
        return (
            self.session.query(ExtractedProfilingData)
            .filter(ExtractedProfilingData.user_id == user_id)
            .order_by(ExtractedProfilingData.created_at.desc())
            .limit(limit)
            .all()
        )

    def get_cpde7_data_by_chat(
        self,
        *,
        user_id: str,
        chat_id: int,
        ...
        return (
            self.session.query(ExtractedProfilingData)
            .filter(ExtractedProfilingData.user_id == user_id)
            .filter(ExtractedProfilingData.chat_id == chat_id)
            .order_by(ExtractedProfilingData.created_at.desc())
            .limit(limit)
            .all()
        )

    def get_cpde7_data_by_dimension(
        self,
        *,
        user_id: str,
        dimension: str,
        ...
        return (
            self.session.query(ExtractedProfilingData)
            .filter(ExtractedProfilingData.user_id == user_id)
            .filter(ExtractedProfilingData.data['dimension'].as_string() == dimension)
            .order_by(ExtractedProfilingData.created_at.desc())
            .limit(limit)
            .all()
        )

    def get_cpde7_data_grouped(self, *, user_id: str, limit_per_dimension: int = 20) -> dict:
        ...
        for dim in dimensions:
            items = self.get_cpde7_data_by_dimension(
    ...
    def delete_cpde7_data_for_chat(self, *, user_id: str, chat_id: int) -> int:

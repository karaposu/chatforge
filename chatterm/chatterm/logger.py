"""
Simple logging wrapper for ChatTerm

Logs all output to a file with timestamps when enabled.
Adapted from VoxTerm - removed audio-specific methods.
"""
...
class SimpleLogger:
    ...
    def enable(self, log_file_path: str):
        ...
        try:
            ...
            self.file_handle.write(
                f"ChatTerm Session Log - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n"
            )
    ...
    def log_middleware_event(self, middleware: str, action: str):
        """Log middleware events"""
        self.log_event("MIDDLEWARE", f"[{middleware}] {action}")

    def log_tool_event(self, tool_name: str, action: str):
        """Log tool invocation events"""
        self.log_event("TOOL", f"[{tool_name}] {action}")
